...
Максимум 200 слов. Не объясняй выводы. Не доказывай выводы. Не используй таблицы и длинные абзацы. Если кандидат явно подходит или явно не подходит — ответ должен быть ещё короче.`,
  ...
  questionsPortrait: `Ты — Senior Recruiter и HR-консультант. По вакансии и резюме кандидата определи:

{VACANCY}

1. Нужный психологический портрет для этой роли: 2-4 ключевые личностные черты. Разные роли требуют разных типов личности — например, менеджеру по продажам нужны амбициозность и активность, сметчику — скрупулёзность и усидчивость. Опирайся на суть роли (характер задач, самостоятельность, давление, монотонность/разнообразие), а не на шаблон.
2. Какие мотивы кандидата (из справочника ниже, используй ТОЛЬКО эти названия, не выдумывай других) для этой роли желательны, а какие рискованны или противопоказаны. Выбери 2-4 приоритетных мотива, важных для диагностики на интервью.

СПРАВОЧНИК МОТИВОВ (по методике Е. Адессерман):
{MOTIVES}

Ответ короткий и по делу: психологический портрет (2-4 черты) + список приоритетных мотивов с однострочным обоснованием каждого. Без вступлений и заключений.`,
...
* уже готовый психологический портрет вакансии и приоритетные мотивы для диагностики (определены заранее отдельным шагом и переданы тебе во входных данных — используй их как есть, не выводи заново).
...
## БЛОК 1. СТРАТЕГИЯ ИНТЕРВЬЮ

Кратко: какой тип кандидата перед нами; 2-3 главных риска, которые нужно проверить; что из сильных сторон требует подтверждения.

## БЛОК 2. КЛЮЧЕВЫЕ ГИПОТЕЗЫ

3-4 гипотезы (не больше). Для каждой: гипотеза; почему возникла; вопрос для проверки; что будет сильным ответом; что будет тревожным ответом.

## БЛОК 3. ВОПРОСЫ ПО ЭТАПАМ ИНТЕРВЬЮ

Разбей на этапы: (1) глубина реального опыта, (2) самостоятельность в решениях, (3) масштаб ответственности (бюджеты/объекты/люди), (4) совместимость с компанией (давление/контроль/конфликты). На каждый этап — 1-2 конкретных вопроса (не общих, а вытекающих из резюме), без длинных списков. Для каждого вопроса — что он проверяет и на что обратить внимание в ответе.

## БЛОК 4. КРАСНЫЕ ФЛАГИ

3-4 пункта: какие ответы должны насторожить (завышение опыта, зависимость от руководителя, недостаток компетенции).

## БЛОК 5. ВОПРОСЫ НА ВЫЯВЛЕНИЕ МОТИВОВ

3-4 вопроса (не больше), которые помогают определить реальные мотивы кандидата — именно те приоритетные мотивы, что переданы тебе во входных данных. Не спрашивай «что вас мотивирует» в лоб — используй поведенческие и проективные вопросы: про конкретные прошлые ситуации, что вызывало энтузиазм или демотивировало, за что кандидат гордится, как реагировал на неудачи/критику/рутину. Для каждого вопроса: какой мотив проверяет; маркер наличия мотива в ответе; маркер отсутствия/противоположного мотива.
...
ОГРАНИЧЕНИЕ ОБЪЁМА: пиши компактно и по делу, без вступлений, повторов и общих фраз. Суммарно по всем блокам — не больше 500 слов. Это важнее полноты.`,
...
Если в переданных материалах есть блок «ПСИХОЛОГИЧЕСКИЙ ПОРТРЕТ И ПРИОРИТЕТНЫЕ МОТИВЫ» (сформированный на этапе подготовки вопросов к интервью) — сравни выявленные мотивы кандидата с приоритетными мотивами для вакансии из этого блока и сделай вывод: совпадают / частично совпадают / не совпадают, и что это значит для решения по кандидату. Если такого блока нет — самостоятельно определи по вакансии, какие мотивы для неё важны, и сделай то же сравнение.
...
function generateQuestions() {
  ...
  c.qFocus = focus;
  const userBase = `АНАЛИЗ РЕЗЮМЕ:\n${c.rawAnalysis}\n\nРЕЗЮМЕ:\n${c.resume}${focus ? '\n\nОСОБЫЙ ФОКУС: ' + focus : ''}`;
  el.innerHTML = '<div class="loading"><span class="dot"></span><span class="dot"></span><span class="dot"></span> Шаг 1/2: определяю психологический портрет под вакансию…</div>';
  callAPI({
    ...
    onSuccess: (portraitText) => {
      ...
      callAPI({
        ...
        onSuccess: (mainText) => {
          ...
          c.rawAnalysisQuestions = fullText;
          c.hasQuestions = true;
          save(); renderCandidates();
          el.innerHTML = c.questionsHTML;
          syncToSheets();
        },
        onError: (msg) => { el.innerHTML = errorBox('Шаг 2 (вопросы) не удался: ' + msg + ' — портрет уже посчитан, попробуй ещё раз, это должно сработать быстрее.'); }
      });
    },
    onError: (msg) => { el.innerHTML = errorBox('Шаг 1 (портрет) не удался: ' + msg); }
  });
}
...
function analyzeInterview() {
  ...
  const questionsBlock = c.rawAnalysisQuestions ? '\n\nМАТЕРИАЛЫ ПОДГОТОВКИ К ИНТЕРВЬЮ (включая психологический портрет и приоритетные мотивы для вакансии):\n' + c.rawAnalysisQuestions : '';
  callAPI({
    system: buildPrompt('interview'),
    user: `АНАЛИЗ РЕЗЮМЕ:\n${c.rawAnalysis}${questionsBlock}\n\nТРАНСКРИПТ ИНТЕРВЬЮ:\n${transcript}`,
    loadingEl: el,
    onSuccess: (text) => {

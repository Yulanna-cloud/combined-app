...
ОБЯЗАТЕЛЬНАЯ ПРОВЕРКА ОТРАСЛЕВОГО СООТВЕТСТВИЯ: определи узкую специализацию кандидата по факту его опыта и образования (например: ПГС/гражданское строительство, электрика/энергетика, промышленное строительство, ВК, механика и т.д.). Сверь её с профилем компании из блока «О КОМПАНИИ» (если он есть). Если компания работает в узкой инженерной нише (например, электротехника, энергетика), а у кандидата нет подтверждённого опыта именно в этой нише — это ОБЯЗАТЕЛЬНО отдельный явный пункт в блоке РИСКИ (не фон, не общая фраза), и это должно понижать решение минимум до 🟡, даже если остальной опыт кандидата сильный. Общий стаж в строительстве НЕ компенсирует отсутствие опыта в нужной инженерной специализации.
Важное уточнение: промышленное строительство ДЛЯ конкретного заказчика/отрасли (например, строительство объектов для нефтегазовой, энергетической и т.п. отрасли) — это ВСЁ РАВНО промышленное/гражданское строительство, а не отраслевое несовпадение. Не путай отрасль конечного заказчика кандидата с его строительной специализацией — если кандидат строил, вёл технадзор, руководил ПТО/участком на промышленных объектах, это опыт в промышленном строительстве, кем бы ни был заказчик.

ОБРАЗОВАНИЕ — ЖЁСТКОЕ ПРАВИЛО (без исключений): формальное название специальности в дипломе (например, «Электроэнергетика», «Механика» и т.п. вместо «ПГС» или «Строительство») САМО ПО СЕБЕ НИКОГДА не является основанием для 🔴 и никогда не входит в блок РИСКИ, если у кандидата есть 3+ года практического опыта на инженерно-строительных должностях (ПТО, прораб, начальник участка, главный инженер, технический надзор и т.п.) — независимо от давности этого опыта. В этом случае строчку про образование вообще не включай ни в РИСКИ, ни в СИЛЬНЫЕ СТОРОНЫ — это нейтральный факт, не влияющий на решение. Реальный опыт всегда важнее диплома. Единственное исключение: если у кандидата вообще нет инженерно-строительного опыта — тогда это можно отметить, но не как единственную причину отказа.

ФОРМАЛЬНЫЕ ТРЕБОВАНИЯ vs КОММЕНТАРИИ РЕКРУТЕРА: если в требованиях вакансии заявлен формальный критерий (образование, конкретная специальность и т.п.), но в комментариях рекрутера есть явная оговорка-исключение («не принципиально, если...», «не обязательно, но желательно», «можно без Х, если есть Y» и т.п.) — комментарии ИМЕЮТ ПРИОРИТЕТ. Не отказывай кандидату только за формальное несоответствие, если у него есть релевантный практический опыт, покрывающий это исключение. Формальное несоответствие в такой ситуации — это повод уточнить на звонке, а не автоматический отказ.

ЧАСТАЯ СМЕНА МЕСТ РАБОТЫ: если в комментариях рекрутера указано, что частая смена мест — красный флаг, проверяй это не формально, а с датировкой. Если частые смены (короче года) были только в ОДИН период карьеры, особенно в прошлом (например, 5+ лет назад), а после этого кандидат работал стабильно — прямо укажи в РИСКАХ этот период («часто менял места в ЧЧЧЧ-ЧЧЧЧ, затем стабилен N лет») вместо общей фразы «часто меняет работу». Не приравнивай единичный давний эпизод к текущей ненадёжности — это разные по весу риски, и решение должно это отражать.
...
Максимум 200 слов. Не объясняй выводы. Не доказывай выводы. Не используй таблицы и длинные абзацы. Если кандидат явно подходит или явно не подходит — ответ должен быть ещё короче.`,
  ...
  questionsPortrait: `Ты — Senior Recruiter. По вакансии и резюме кандидата определи 2-4 приоритетных мотива кандидата (из справочника ниже, используй ТОЛЬКО эти названия), которые важно проверить на интервью именно для этой роли.

{VACANCY}

СПРАВОЧНИК МОТИВОВ (по методике Е. Адессерман):
{MOTIVES}

Составь 3-4 вопроса для интервью, которые помогают определить эти мотивы у кандидата. Не спрашивай «что вас мотивирует» в лоб — используй поведенческие и проективные вопросы: про конкретные прошлые ситуации, что вызывало энтузиазм или демотивировало, за что кандидат гордится, как реагировал на неудачи/критику/рутину.

СТРОГИЙ ФОРМАТ ОТВЕТА (и ничего сверх этого — без портрета, без обоснований, без таблиц, без вступлений и заключений):

МОТИВЫ ДЛЯ ПРОВЕРКИ: [название], [название], [название]

ВОПРОСЫ:
1. [вопрос] — проверяет: [мотив]
2. [вопрос] — проверяет: [мотив]
3. [вопрос] — проверяет: [мотив]
4. [вопрос] — проверяет: [мотив]

Максимум 150 слов.`,

  questions: `Ты — опытный рекрутер. Готовишь вопросы для интервью под конкретного кандидата — не шаблонные, а такие, что вытекают из его резюме и предыдущего анализа.

{VACANCY}

Каждый вопрос должен проверять конкретный риск, пробел или гипотезу, возникшую после анализа резюме этого кандидата. Общие вопросы («расскажите о себе», «почему хотите работать у нас») не нужны.

СТРОГИЙ ФОРМАТ ОТВЕТА — и ничего сверх этого. Никакого анализа кандидата, стратегии, гипотез-таблиц, обоснований, вступлений и заключений. Только вопросы:

ОПЫТ И ГЛУБИНА
1. [вопрос]
2. [вопрос]

САМОСТОЯТЕЛЬНОСТЬ И МАСШТАБ ОТВЕТСТВЕННОСТИ
1. [вопрос]
2. [вопрос]

СОВМЕСТИМОСТЬ С КОМПАНИЕЙ (давление, контроль, конфликты)
1. [вопрос]
2. [вопрос]

КРАСНЫЕ ФЛАГИ — на что обратить внимание в ответах
- [короткий пункт]
- [короткий пункт]

Максимум 10-12 вопросов всего. Каждый вопрос — одна строка, без пояснений после него. Максимум 250 слов на весь ответ.`,
...
4. МОТИВЫ КАНДИДАТА
Определи по транскрипту 2-3 ведущих мотива кандидата, используя ТОЛЬКО названия и формулировки из справочника ниже (не выдумывай других мотивов, не путай с типом темперамента или компетенциями). Для каждого найденного мотива приведи короткую цитату/признак из транскрипта, на основании которого сделан вывод.

Если в переданных материалах есть блок «МОТИВЫ, ПРИОРИТЕТНЫЕ ДЛЯ ЭТОЙ ВАКАНСИИ» — сравни выявленные мотивы кандидата с приоритетными мотивами для вакансии из этого блока и сделай вывод: совпадают / частично совпадают / не совпадают, и что это значит для решения по кандидату. Если такого блока нет — самостоятельно определи по вакансии, какие мотивы для неё важны, и сделай то же сравнение.
...
ОГРАНИЧЕНИЕ ОБЪЁМА: пиши компактно, без вступлений и повторов. Суммарно по всем 7 пунктам — не больше 600 слов. Это важнее полноты — лучше кратко и по делу, чем развёрнуто.`,

  chatAnswers: `Ты — строгий, требовательный рекрутер. Кандидату в чате HH.ru были отправлены уточняющие вопросы по резюме (текст вопросов ты найдёшь в анализе резюме, в разделе «ВОПРОСЫ ДЛЯ СКРИНИНГА В ЧАТ HH»), и он на них ответил.
...
ВАЖНО — разделяй риски на две группы:
1. Риски, по которым кандидату РЕАЛЬНО был задан вопрос в разделе «ВОПРОСЫ ДЛЯ СКРИНИНГА В ЧАТ HH» — только их разбирай как закрыт/не закрыт/частично на основе ответа.
2. Риски, которые упомянуты в общем списке РИСКОВ анализа резюме, но НЕ вошли в вопросы для чата — по ним кандидат ничего не игнорировал и не обязан был отвечать. НЕ пиши по ним "не затронут" и не засчитывай как повод для отказа. Вынеси их отдельным списком "не проверено скринингом — уточнить на звонке" и не включай в обоснование вердикта "не звать", если только это не risk настолько критичный, что перевешивает всё остальное (тогда явно поясни, что это самостоятельная причина, а не следствие ответов кандидата).

НЕ ТРЕБУЙ ИЗБЫТОЧНЫХ ДОКАЗАТЕЛЬСТВ: если вопрос касается простого проверяемого факта (состоит ли кандидат в реестре/аттестации, есть ли документ, гражданство и т.п.) и кандидат дал прямой ответ "да"/"нет" — этого достаточно, чтобы считать риск закрытым. Не занижай вердикт из-за отсутствия номера записи, скриншота или другого документального подтверждения в чате — переписка HH технически не всегда позволяет прикладывать файлы, и врать в таких простых проверяемых фактах кандидату нет смысла (легко проверяется на следующих этапах). Требуй документальное подтверждение только если сам ответ выглядит уклончивым, противоречивым или неуверенным по смыслу.

Если ответ кандидата на заданный вопрос сам порождает НОВЫЙ риск (например, опыт не в той сфере, не он лично делал работу) — это засчитывается как риск группы 1, не как смягчение.

Дай вердикт:

1. ВЕРДИКТ — звать на интервью / не звать (однозначно, без "можно бы, но")
2. РАЗБОР ПО ЗАДАННЫМ ВОПРОСАМ — для каждого риска, который реально был вынесен в вопрос для чата: закрыт / не закрыт / частично, и почему
3. НЕ ПРОВЕРЕНО СКРИНИНГОМ — риски из анализа резюме, которые не задавались в чате; коротко, без влияния на вердикт (кроме явно исключительных случаев — тогда обязательно поясни это отдельно)
4. ЕСЛИ ЗВАТЬ — единственное, что ещё нужно прояснить на интервью (если ничего критичного не осталось — так и скажи)
...
// ── Справочник мотивов (по методике Е. Адессерман, сжато для экономии
// токенов — полное описание каждого мотива в промт не отправляется) ────
const MOTIVES_REFERENCE = `ПРИЗНАНИЕ: важно, что его работа/мнение/он сам ценятся; в речи много «я», историй успеха, ссылок на авторитетов.
ПОХВАЛА (польза): важно быть нужным и полезным, а не просто заметным; зависим от мнения коллег/клиентов; легко принимает конструктивную критику.
СТАТУС: важны должность, кабинет, власть, право быть правым; низкая обучаемость, плохо принимает критику и чужой авторитет.
КОЛЛЕКТИВ (команда): оценка команды важнее личных интересов; нужен хороший микроклимат; плохой автономный руководитель, хорош как линейный лидер.
ПРОФЕССИОНАЛИЗМ: серьёзное отношение к своему делу/ремеслу, стремление к мастерству; хорошо воспринимает критику по делу, плохо — обвинения в непрофессионализме.
АЗАРТ: нужен вызов и измеримая цель, любит выигрывать и быть замеченным в этом; тяжело переносит поражения и рутину, нетерпелив к наставничеству.
КАРЬЕРА: нужен рост именно по вертикали (должность, статус компании); встречается редко; легко обучаем, конкурирует с руководителем.
РЕЗУЛЬТАТ: важен чёткий план и видимая точка результата впереди; риск гиперконтроля и сложностей с эмпатией к менее замотивированным.
ЛИЧНОСТЬ РУКОВОДИТЕЛЯ: лоялен не любому начальнику, а тому, кто соответствует его «образу хорошего руководителя»; тяжело переживает смену руководства.
ИДЕЯ: важно, чтобы работа служила значимой для него идее/миссии; плохо подходит для рутины и чистой аналитики; почти не демотивируется, если идея жива.
РАЗВИТИЕ: постоянная потребность узнавать новое, легко относится к переменам; плохо переносит рутину и стабильную устоявшуюся среду.
ЧЁТКИЕ ЦЕЛИ: важен не результат сам по себе, а понятная последовательность действий и разделение ответственности; хорош в регулярном менеджменте.
НА РАВНЫХ: важны отношения на основе взаимоуважения с руководством и коллегами; чутко считывает давление и манипуляции, не терпит авторитарный стиль.
СПРАВЕДЛИВОСТЬ: важно, чтобы оценка/оплата/признание были заслуженными (и для него, и для окружающих); тяжело — необоснованная критика и неочевидные критерии роста.
СТАБИЛЬНОСТЬ: важны предсказуемость и безопасность будущего; тяжело даются перемены, смена руководителя, смена зоны ответственности.
ИЗБЕГАНИЕ: стремление уйти от негативного опыта и рисков; в речи много «не» и акцентов на проблемы; плохо берёт на себя ответственность за чужие действия.
КОМФОРТ: важны формальные условия работы (дорога, график, обстановка); легко переманить лучшими условиями; часто говорит, что «главное — деньги».
ВЛАСТЬ: постоянная потребность демонстрировать превосходство в любой коммуникации; низкая рефлексия, сложно принимает чужую правоту, тяжёлый для управления.
САМОРЕАЛИЗАЦИЯ: высший уровень — своё, ценностное представление об успехе; высокая автономность и самомотивация; не способен к рутинной работе.`;
...
// ── API ───────────────────────────────────────────────────────────
let _apiBusy = false;
async function callAPI({ system, user, loadingEl, onSuccess, onError, maxTokens, timeoutMs }) {
  ...
  try {
    ...
    const timeoutId = setTimeout(() => controller.abort(), timeoutMs || 90000); // не ждём дольше 90 секунд (можно переопределить)
    let resp;
    try {
    ...
    const msg = e.name === 'AbortError'
      ? `ИИ не ответил за ${Math.round((timeoutMs || 90000) / 1000)} секунд (похоже на перегрузку сервиса). Попробуй ещё раз.`
...
// Отдельный короткий инструмент: только психологический портрет вакансии +
// приоритетные мотивы + несколько вопросов на их выявление. Не смешивается
// с основным анализом вопросов — короткий ответ, быстрый запрос.
function generateMotivesForVacancy() {
  const c = currentCandidate(); if (!c) return;
  const el = document.getElementById('motives-vacancy-content');
  callAPI({
    ...
    timeoutMs: 60000,
    onSuccess: (text) => {
      c.motivesVacancyHTML = resultBox('Мотивы под вакансию · ' + c.name, text);
      c.rawMotivesVacancy = text;
      save(); renderCandidates();
      el.innerHTML = c.motivesVacancyHTML;
      syncToSheets();
    },
    onError: (msg) => { el.innerHTML = errorBox(msg); }
  });
...
function analyzeInterview() {
  ...
  const motivesBlock = c.rawMotivesVacancy ? '\n\nМОТИВЫ, ПРИОРИТЕТНЫЕ ДЛЯ ЭТОЙ ВАКАНСИИ (определены заранее):\n' + c.rawMotivesVacancy : '';
  callAPI({
    system: buildPrompt('interview'),
    user: `АНАЛИЗ РЕЗЮМЕ:\n${c.rawAnalysis}${motivesBlock}\n\nТРАНСКРИПТ ИНТЕРВЬЮ:\n${transcript}`,
    loadingEl: el,
    maxTokens: 1800,
    timeoutMs: 150000,
    onSuccess: (text) => {
